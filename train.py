...
def validate_one_epoch(model, dataloader, criterion, current_epoch_config, current_epoch_num, global_logger, rank, world_size):
    ...
    if rank == 0: 
        ...
        if all_preds_sigmoid_gathered is not None and all_targets_gathered is not None and all_preds_sigmoid_gathered.shape[0] > 0:
            ...
            global_logger.error(f"Epoch {current_epoch_num} [验证] 无法收集到足够的预测或目标来进行指标计算。")
    ...
    try: 
        for epoch in range(start_epoch, current_run_config.EPOCHS):
            ...
            if world_size > 1 and dist.is_initialized():
                metric_device = current_run_config.DEVICE if isinstance(current_run_config.DEVICE, str) else f'cuda:{current_run_config.DEVICE}'
                metric_tensor = torch.tensor(val_metric_f1_micro_rank0, device=metric_device)
