...
BASE_PROCESSING_COLUMNS_FOR_CSV = list(csv_aggregator.TAG_COLUMN_TO_CATEGORY.keys()) + [RATING_COLUMN_NAME]
DOWNLOADER_SPECIFIC_COLS_TO_READ = [ID_COLUMN_NAME, TAG_COLUMN_FOR_TXT_CAPTIONS, RATING_COLUMN_NAME]
...
if RATING_COLUMN_NAME not in ALL_REQUIRED_COLUMNS_IN_PARQUET:
    ALL_REQUIRED_COLUMNS_IN_PARQUET.append(RATING_COLUMN_NAME)
...
def fetch_data_and_prepare_for_processing(parquet_file_obj,
    ...
    def process_dataframe_chunk(df_chunk_to_process_param):
        ...
        if RATING_COLUMN_NAME not in df_chunk_to_process.columns: # 确保rating列存在，否则赋None
            df_chunk_to_process.loc[:, RATING_COLUMN_NAME] = None
        ...
        for _, row_series in filtered_rows_df.iterrows():
            ...
            if int_id is not None and int_id not in processed_int_ids_for_downloader_dedup:
                ...
                rating_value = row_series.get(RATING_COLUMN_NAME) # 获取rating值
                downloader_data_list.append({
                    'id': int_id,
                    'caption_tags': caption_tag_string,
                    'rating': rating_value # 存储rating值
                })
    ...
    for item in valid_items_for_captions:
        ...
        if os.path.exists(image_file_path):
            ...
            if pd.notna(raw_caption_tags) and str(raw_caption_tags).strip():
                tags_list.extend([tag.strip() for tag in str(raw_caption_tags).strip().replace(' ', ',').split(',') if tag.strip()])

            if pd.notna(rating_char):
                rating_name = csv_aggregator.RATING_TO_NAME.get(str(rating_char).lower())
                if rating_name:
                    tags_list.append(rating_name)
            
            tags_for_file = ",".join(tags_list)
...
def main_downloader_and_processor():
    ...
    print(f".txt标签内容来自Parquet列: '{TAG_COLUMN_FOR_TXT_CAPTIONS}' (并将包括 '{RATING_COLUMN_NAME}' 标签)") 
    ...
    try:
        ...
        if TAG_COLUMN_FOR_TXT_CAPTIONS not in schema_names:
            print(f"警告：用于.txt文件的标签列 '{TAG_COLUMN_FOR_TXT_CAPTIONS}' 在Parquet文件中不存在")
        if RATING_COLUMN_NAME not in schema_names: 
             print(f"警告：用于.txt文件的 '{RATING_COLUMN_NAME}' 列在Parquet文件中不存在。rating标签将不会被添加")
