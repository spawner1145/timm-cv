...
class Eva02LargeClip336Merged2BConfig(BaseConfig):
    ...
    # 模型配置
    MODEL_NAME = "eva02_base_patch16_clip_224.merged2b" # 确切的模型名称，你可以通过运行get_modelnames.py来获取
    ...
    ACCUMULATION_STEPS = 5 # 示例：等效批次大小为 4*8=32,如果BATCH_SIZE更小，相应增加此值
    
    LEARNING_RATE = 2e-5 # 微调大型 CLIP 模型常用的学习率范围：5e-6 到 2e-5
    WEIGHT_DECAY = 0.05  # AdamW 常用的权重衰减值
    
    EPOCHS = 100 # 微调时通常不需要像从头训练那么多轮次，可以从较少的轮次开始实验
                # 具体轮数取决于你的数据集大小和收敛速度
